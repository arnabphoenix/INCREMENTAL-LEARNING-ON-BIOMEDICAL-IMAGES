...
def main(argv=None):
    ...
    # dataset args
    parser.add_argument('--datasets', default=['bloodmnist'], type=str, choices=list(dataset_config.keys()),
    ...
    # training args
    parser.add_argument('--approach', default='a-icarl', type=str, choices=approach.__all__,
    ...
    utils.seed_everything(seed=args.seed)
    #init_model = SEResNet18(num_classes=some_value)
    ...
    for t, (_, ncla) in enumerate(taskcla):
        ...
        # Train
        # pdb.set_trace()
        labels = torch.tensor(trn_loader[t].dataset.labels)
        unique_lbls = torch.unique(labels)
        frequencies = torch.Tensor([len(torch.where(labels==lbl)[0]) for lbl in unique_lbls]).cuda().float()
        all_fequencies.append(frequencies)
        weights = torch.cat(all_fequencies, dim=-1)
        weights = weights / weights.max()
        weights = 1. / weights
        weights[weights!=0] = weights[weights!=0]
        # weights[weights!=0] = weights[weights!=0] * 20
        # weights = F.softmin(weights, dim=-1)
        appr.weights = weights
        # appr.weights = None
