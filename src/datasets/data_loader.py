...
def get_loaders(datasets, num_tasks, nc_first_task, batch_size, num_workers, pin_memory, validation=.1):
    ...
    for idx_dataset, cur_dataset in enumerate(datasets, 0):
        ...
        for tt in range(num_tasks):
            ...
            # labels = torch.tensor(trn_dset[tt].labels)
            # unique_lbls = torch.unique(labels).tolist()
            # weights = torch.Tensor([len(torch.where(labels==lbl)[0]) for lbl in unique_lbls]).cuda().float()
            # weights = weights / weights.max()
            # weights = F.softmin(weights, dim=-1)
            # sample_weights = [0]*len(trn_dset[tt])

            # for idx, (imgs, lbl) in enumerate(trn_dset[tt]):
            #     class_weights = weights[unique_lbls.index(lbl)]
            #     sample_weights[idx] = class_weights
            
            # sampler = WeightedRandomSampler(sample_weights, len(sample_weights), replacement=True )
            # trn_load.append(data.DataLoader(trn_dset[tt], batch_size=batch_size, sampler = sampler, num_workers=num_workers,
