...
class Appr(Inc_Learning_Appr):
    ...
    def criterion(self, t, outputs, targets):
        ...
        if self.all_out or len(self.exemplars_dataset) > 0:
            return torch.nn.functional.cross_entropy(torch.cat(outputs, dim=1), targets, weight=self.weights)
        return torch.nn.functional.cross_entropy(outputs[t], targets - self.model.task_offset[t], weight=self.weights[self.model.task_offset[t]:])
