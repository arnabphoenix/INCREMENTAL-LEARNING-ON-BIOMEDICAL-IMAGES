...
class Appr(Inc_Learning_Appr):
    ...
    def __init__(self, model, device, nepochs=60, lr=0.5, lr_min=1e-4, lr_factor=3, lr_patience=5, clipgrad=10000,
                 ...
                 eval_on_train=False, logger=None, exemplars_dataset=None, lamb=10, lamb_a = 1, T = 2):
    ...
    def criterion(self, t, outputs, targets, targets_old = None, targets_aux = None):
        ...
        targets = targets.to(torch.long)
        loss = torch.nn.functional.cross_entropy(torch.cat(outputs, dim=1), targets, weight=self.weights)
        ...
        if t > 0:
            ...
            q_i = torch.sigmoid(torch.cat(targets_old[:t], dim=1))
            loss += self.lamb * sum(torch.nn.functional.binary_cross_entropy(g[:, y], q_i[:, y]) for y in
                                    range(sum(self.model.task_cls[:t])))

            g = torch.sigmoid(torch.cat(outputs[:t], dim=1))
            q_i = torch.sigmoid(torch.cat(targets_aux[:t], dim=1))

            loss += self.lamb_a * sum(torch.nn.functional.binary_cross_entropy(g[:, y], q_i[:,y])for y in
                                      range(sum(self.model.task_cls[:t])))
